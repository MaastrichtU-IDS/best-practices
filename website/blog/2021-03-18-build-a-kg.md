...
* [**Ontologies**](https://www.w3.org/OWL/), aka. vocabularies: data models for knowledge graphs, they describe their classes and properties. e.g. the class https://schema.org/Restaurant in Schema.org vocabulary
...
* [**YARRRML**](https://rml.io/yarrrml/spec/): a simplification of RML to define mappings in the popular human-readable [YAML format](https://yaml.org/), easier to define than the RML RDF.
...
5. If we want to be more semantically accurate, we can add a triple to indicate that entities with the type `schema:Restaurant` are also considered as `fo:Restaurant`. This improves your knowledge graph semantics, since the [`fo:cuisine`](https://www.bbc.co.uk/ontologies/fo#terms_cuisine) is not defined as a property of `schema:Restaurant` in the Schema.org vocabulary. 

We can use the standard `owl:equivalentClass` property to indicate the 2 ontology classes are equivalent with a triple:

```turtle
<https://schema.org/Restaurant> <http://www.w3.org/2002/07/owl#equivalentClass> <http://purl.org/ontology/fo/Restaurant> .
```
...
You can also create [SHACL](https://www.w3.org/TR/shacl/) or [ShEx](https://shex.io/) shapes for your knowledge graph entities, this will allow you to validate the data added to your knowledge graph to make sure it meets your requirements
